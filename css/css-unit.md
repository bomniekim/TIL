...
`%`는 백분률 단위의 상대 단위이다. 요소에 지정된 사이즈(부모 요소의 사이즈(상속된 사이즈)나 default 사이즈)에 영향을 받아 그에 대한 상대적인 사이즈를 설정한다. 

<br>

## 3. `em`

`em`은 배수(倍數) 단위의 상대 단위이다. em 단위는 상위 요소 크기의 몇 배인지로 사이즈를 설정한다.

예를 들어 _1em_ 은 상위 요소에 지정된 사이즈와 같고 _2em_ 은 요소에 지정된 사이즈의 _2배_ 이다.
...
`rem`은 __최상위 요소(html)의 사이즈__ 를 기준으로 삼는 상대 단위이다. rem은 em과 달리 최상위 부모의 값(하나)을 기준으로 하기 때문에 중첩된 자식 요소에서도 크기를 지정하는데 혼동없이 사용할 수 있다.
